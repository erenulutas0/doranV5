...
CREATE INDEX IF NOT EXISTS idx_shops_location_point ON shops USING GIST(location_point) 
WHERE location_point IS NOT NULL;

-- Function to update location_point when latitude/longitude changes
CREATE OR REPLACE FUNCTION update_shop_location_point()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_update_shop_location_point ON shops;
CREATE TRIGGER trigger_update_shop_location_point
    BEFORE INSERT OR UPDATE OF latitude, longitude ON shops
    FOR EACH ROW
    EXECUTE FUNCTION update_shop_location_point();
